 ...
 *         getArtistByIndex returns a string in the format `The artist at
 *         index {id} is {name}.`
 *
 * For example, if getArtistByIndex is invoked with the artists
 * dataset and the number 0, it will return `The artist at index 0 is
 * Amedeo Modigliani`.
 */
 ...
 * array until there are none left. If you refresh your browser, the
 * data will reset.
 */
...
function randomize(data){
    ...
    for (_ = 0; _ < dataLength; _++) {
        const randomIndex = Math.floor(Math.random() * data.length);
        const item = data.splice(randomIndex, 1)[0];
        newData.push(item);
    }
